...
parseStatus :: B.ByteString -> GameStatus
  ...
  where
    parseStatus' = do
      ...
      when (not empty) $ do
        ...
        fail $ printf "parseStatus: %d bytes remain unread" $ BL.length rem

      return GameStatus { name       = name,
                          state      = gameState,
                          turn       = turn,
    ...
    parseNation nth (player, submitted, connected) =
      Just PlayerStatus { nationId  = nth,
